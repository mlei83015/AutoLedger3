...
public class QuickAddActivity extends Activity {
    ...
    private void build() {
        ...
        root.addView(presetStrip(income, category, accent), lp(-1, dp(42), 0, dp(4), 0, dp(12)));
    ...
    private HorizontalScrollView presetStrip(boolean income, EditText category, int accent) {
        HorizontalScrollView hsv = new HorizontalScrollView(this);
        hsv.setHorizontalScrollBarEnabled(false);
        hsv.setOverScrollMode(android.view.View.OVER_SCROLL_NEVER);
        hsv.setClipToPadding(false);
        hsv.setPadding(dp(2), 0, dp(2), 0);
        ...
        if (presets == null || presets.isEmpty()) {
            ...
            String[] defaults = income
                    ...
                    : new String[]{"飲料", "點心", "早餐", "午餐", "晚餐", "交通", "停車", "全聯"};
            for (String d : defaults) presets.add(d);
        }
        for (String raw : presets) {
            String name = cleanQuickName(raw);
            ...
            LinearLayout.LayoutParams chipLp = new LinearLayout.LayoutParams(dp(82), dp(38));
            chipLp.setMargins(0, 0, dp(8), 0);
            row.addView(b, chipLp);
        }
        Button add = button("＋新增", CARD_SOFT, 0xFF7EE8FF, 12, dp(18));
        add.setSingleLine(true);
        add.setOnClickListener(v -> showAddQuickItemDialog(income));
        LinearLayout.LayoutParams addLp = new LinearLayout.LayoutParams(dp(82), dp(38));
    ...
    private void showAddQuickItemDialog(boolean income) {
        ...
        panel.setBackground(round(BG, dp(24), BORDER));
        panel.addView(t(income ? "新增收入常用項目" : "新增支出常用項目", 20, TEXT, true));
        TextView desc = t("只輸入名稱就好，金額每次自己打；新增後會放在快速常用項目裡。", 13, MUTED, false);
        panel.addView(desc, lp(-1, -2, 0, dp(4), 0, dp(10)));
        EditText input = input(income ? "例如 薪水、零用錢" : "例如 早餐、飲料、交通", false, income ? PURPLE : RED, "");
        input.setSingleLine(true);
        panel.addView(input, lp(-1, dp(50), 0, 0, 0, dp(12)));
        LinearLayout actions = new LinearLayout(this);
        actions.setOrientation(LinearLayout.HORIZONTAL);
        Button cancel = button("取消", CARD_SOFT, TEXT, 14, dp(18));
        Button save = button("✓ 儲存", ORANGE, Color.WHITE, 14, dp(18));
        actions.addView(cancel, weighted(0, dp(46), 1, 0, 0, dp(6), 0));
        actions.addView(save, weighted(0, dp(46), 1, dp(6), 0, 0, 0));
    ...
    private String cleanQuickName(String line) {
        if (line == null) return "";
        String[] p = line.split("\\|");
