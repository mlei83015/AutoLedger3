...
public class FinanceParser {
    ...
    private static AmountInfo analyzeAmounts(String s) {
        ...
        int explicitLinePayPoints = linePayPointDiscount(s);
        ...
        // 但只在沒有明確點數 / 折抵數字時才使用，避免「使用點數 3 點」被誤判成實付 3 元。
        if (containsDiscountWord(s) && all.size() >= 2 && info.discountAmount <= 0) {
    ...
    private static boolean containsAnyIgnoreCase(String s, String... keys) {
