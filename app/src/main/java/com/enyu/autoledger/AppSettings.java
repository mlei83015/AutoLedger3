...
public class AppSettings {
    ...
    public static final String KEY_ONBOARDED = "onboarded_v38";
    ...
    public static int getMonthlyBalanceAdjust(Context c) {
        return sp(c).getInt(KEY_MONTHLY_BALANCE_ADJUST_PREFIX + currentMonthKey(), 0);
    }

    public static int getMonthlyOpeningSpent(Context c) {
    ...
    public static List<String> getFixedRecords(Context c) {
        return getList(c, KEY_FIXED_RECORDS, "支出|房租|12000|固定支出|每月 1 號\n支出|Netflix|390|訂閱|每月 10 號\n收入|薪水|32000|薪水|每月 5 號");
    }
    ...
    public static List<String> getCustomIcons(Context c) {
        return getList(c, KEY_CUSTOM_ICONS, "");
    }

    public static void addCustomIcon(Context c, String emoji, String label) {
        String ic = emoji == null ? "" : emoji.trim();
        String name = label == null ? "" : label.trim();
        if (ic.isEmpty()) return;
        if (name.isEmpty()) name = "自訂";
        List<String> items = getCustomIcons(c);
        LinkedHashSet<String> set = new LinkedHashSet<>();
        for (String item : items) {
        ...
        set.add(ic + "|" + name);
        setList(c, KEY_CUSTOM_ICONS, new ArrayList<>(set));
    }
